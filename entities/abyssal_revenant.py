...
class AbyssalRevenant(PhysicsEntity):
    def __init__(self, pos: Vector) -> None:
        ...
        spritesheet = SpriteSheet(
                    os.path.join("assets", "abyssal_revenant", "ABYSSAL_REVENANT.png"), rows=5, cols=23)
    ...
    def render(self, canvas: simplegui.Canvas) -> None:
        self.__animations.render(canvas, self._pos, self._size)
    ...
    def interaction(self, player: Player):
        ...
        distance_x = self._pos.x - player._pos.x
        self.take_damage(1)
        print(self.health)
