...
from entities import SIZE, AbyssalRevenant, Block, Mage
...
class LevelEditor(GameLoop):
    ...
    def __set_text(self) -> None:
        ...
        self.__labels[13].set_text("[9] toggle lines")
            ...
            if self.__type == "block":
                ...
                if not changes:
                    if self.__current == "AbyssalRevenant":
                        self._enemies.append(AbyssalRevenant(pos=Vector(x, y)))
                    if self.__current == "Mage":
                        self._enemies.append(Mage(pos=Vector(x, y)))
